from heapq import heappop, heappush

import numpy as np
import pygame
import pygame_widgets

#Thực hiện chuổi bước đi trên UI, cập nhập bộ đếm bước & các qidget
def play_solution(solution, game, widgets, show_solution, moves):
	for move in solution:
		events = pygame.event.get()
		moves += game.player.update(move)
		game.floor_group.draw(game.window)
		game.goal_group.draw(game.window)
		game.object_group.draw(game.window)
		pygame_widgets.update(events)
		widgets['label'].draw()
		widgets['visualizer'].draw()
		widgets['moves_label'].set_moves(f' Moves - {moves} ', 20)
		if show_solution:
			widgets['paths'].draw()
		pygame.display.update()
		pygame.time.delay(130)
	return moves


def print_state(state, shape):
	if not state:
		return
	m, n = shape
	matrix = np.array(list(state)).reshape(m, n)
	print(matrix)


def find_boxes_and_goals(state, shape):
	_, width = shape
	boxes, goals, boxes_on_goal = [], [], []
	for pos, char in enumerate(state):
		if char == '@':
			boxes.append((pos // width, pos % width))
		elif char in 'X%':
			goals.append((pos // width, pos % width))
		elif char == '$':
			boxes_on_goal.append((pos // width, pos % width))
	return boxes, goals, boxes_on_goal


def get_state(matrix):
	return matrix.tobytes().decode('utf-8').replace('\x00', '')


def is_solved(state):
	return '@' not in state


def manhattan_sum(state, player_pos, shape):
	height, width = shape
	player_x, player_y = player_pos
	boxes, goals, _ = find_boxes_and_goals(state, shape)
	boxes_cost = len(boxes) * height * width
	player_cost = 0
	for box_x, box_y in boxes:
		boxes_cost += min(abs(box_x - goal_x) + abs(box_y - goal_y) 
						  for goal_x, goal_y in goals)
	player_cost = min(abs(box_x - player_x) + abs(box_y - player_y) 
					  for box_x, box_y in boxes) if boxes else 0
	return boxes_cost + player_cost

# Tính ma trận khoảng cách ngắn nhất(4 hướng , mỗi bước là 1) từ 1 nguồn đến mọi ô hợp lệ
#Nguồn có thể là box hoặc là player
def dijkstra(state, shape, box_pos=None, player_pos=None):
	height, width = shape
	dijk = np.array([[float('inf') for _ in range(width)] for _ in range(height)])
	dijk[box_pos or player_pos] = 0
	moves = [(1, 0), (-1, 0), (0, 1), (0, -1)]
	heap = [(0, box_pos or player_pos)]
	obstacles = '+' if player_pos else '+@$' # Nếu là người thì cấm đi vào tường, # Nếu là hộp cấm đi vào tường,hộp
	while heap:
		distance, curr_pos = heappop(heap) #pop các ô có khoảng cách nhỏ nhất,nếu đã có giá trị tốt hơn thì bỏ qua còn không xét 4 ô bên cạnh
		if distance > dijk[curr_pos]:
			continue # Thằng nào đi trước đã cập nhập ô này
		for move in moves:
			new_x, new_y = curr_pos[0] + move[0], curr_pos[1] + move[1]
			new_pos = new_x, new_y
			if (1 <= new_x < height - 1 and
				1 <= new_y < width - 1 and
				state[new_x * width + new_y] not in obstacles):
				new_distance = distance + 1
				if new_distance < dijk[new_pos]:
					dijk[new_pos] = new_distance
					heappush(heap, (new_distance, new_pos))
	return dijk


def dijkstra_sum(state, player_pos, shape, distances):
	height, width = shape
	boxes, goals, boxes_on_goal = find_boxes_and_goals(state, shape)
	boxes_cost = len(boxes) * height * width
	player_cost = 0
	for box in boxes + boxes_on_goal:
		distances[box] = dijkstra(state, shape, box)
	distances[player_pos] = dijkstra(state, shape, player_pos=player_pos)
	for box in boxes:
		boxes_cost += min(distances[box][goal] for goal in goals)
	player_cost = min(distances[player_pos][box] for box in boxes) if boxes else 0
	return boxes_cost + player_cost

from .hungarian import minimum_matching_cost as hungarian_algorithm

def minimum_matching_sum(state, player_pos, shape, distances):
	"""
	Tính tổng chi phí tối thiểu cho bài toán gán hộp vào mục tiêu (admissible, nhanh).

	Tối ưu hiệu năng so với phiên bản trước:
	- Dùng trường khoảng cách tĩnh theo tường (goal-centric BFS/Dijkstra với obstacles='+'),
	  do đó KHÔNG phụ thuộc vị trí hộp. Trường này được cache theo mỗi mục tiêu và tái sử dụng
	  cho mọi node. Nhờ vậy, chi phí lập ma trận = O(#boxes * #goals), thay vì chạy Dijkstra
	  cho mỗi hộp ở mỗi node.
	- Vẫn giữ cache kết quả Hungarian theo tập (boxes, goals) để tái sử dụng giữa các node.
	"""
	height, width = shape
	boxes, goals, boxes_on_goal = find_boxes_and_goals(state, shape)

	# Nếu không có hộp hoặc không có mục tiêu, trả về 0
	if not boxes or not goals:
		return 0

	n_boxes = len(boxes)
	n_goals = len(goals)

	# Cache container
	if distances is None:
		distances = {}
	hcache = distances.setdefault('hungarian_cache', {})
	# goal-centric static distance fields: obstacles = '+' (bỏ qua hộp/người)
	gcache = distances.setdefault('goal_static_dists', {})  # key: goal_pos -> 2D np.array

	boxes_key = tuple(sorted(boxes))
	goals_key = tuple(sorted(goals))
	cache_key = (boxes_key, goals_key)

	min_cost = hcache.get(cache_key)
	if min_cost is None:
		# Đảm bảo đã có distance field cho từng goal
		for goal in goals:
			if goal not in gcache:
				# dùng player_pos=goal để obstacles chỉ là tường '+'
				gcache[goal] = dijkstra(state, shape, player_pos=goal)

		# Tạo ma trận chi phí (hộp x mục tiêu) bằng cách tra cứu nhanh
		cost_matrix = []
		big = height * width * 100
		for box in boxes:
			row = []
			for goal in goals:
				d = gcache[goal][box]
				row.append(d if np.isfinite(d) else big)
			cost_matrix.append(row)

		# Cân bằng ma trận (vuông hóa) nếu lệch số lượng
		if n_boxes > n_goals:
			for i in range(n_boxes):
				for _ in range(n_goals, n_boxes):
					cost_matrix[i].append(height * width)
		elif n_goals > n_boxes:
			for _ in range(n_boxes, n_goals):
				cost_matrix.append([height * width] * n_goals)

		# Ma trận vuông cho Hungarian
		n = max(n_boxes, n_goals)
		square_matrix = [[0] * n for _ in range(n)]
		for i in range(n):
			for j in range(n):
				if i < len(cost_matrix) and j < len(cost_matrix[i]):
					square_matrix[i][j] = cost_matrix[i][j]
				else:
					square_matrix[i][j] = height * width

		# Áp dụng Hungarian (trả về chi phí tối thiểu)
		min_cost = hungarian_algorithm(square_matrix)
		hcache[cache_key] = min_cost

	# Thêm chi phí người chơi (tới hộp gần nhất) với cache theo vị trí người
	player_cost = 0
	if boxes:
		pcache = distances.setdefault('player_dijkstra', {})
		player_distances = pcache.get(player_pos)
		if player_distances is None:
			player_distances = dijkstra(state, shape, player_pos=player_pos)
			pcache[player_pos] = player_distances
		player_cost = min(player_distances[box] for box in boxes)

	return min_cost + player_cost



def is_deadlock(state, shape):
	height, width = shape
	if not state or len(state) != height * width:
		return False
	boxes, _, _ = find_boxes_and_goals(state, shape)
	for bx, by in boxes:  # corner deadlock
		box = bx * width + by
		if ((state[box - 1] == '+' and state[box - width] == '+') or # trái và trên
			(state[box + 1] == '+' and state[box + width] == '+') or #phải và dưới
			(state[box + 1] == '+' and state[box - width] == '+') or #phải và trên
			(state[box - 1] == '+' and state[box + width] == '+')):  # trái và dưới
			return True
	double_box_positions = [
		(0, -1, -width, -width - 1), #góc trên trái 
		(0, 1, -width, -width + 1), # góc trên phải
		(0, -1, width - 1, width), # góc dưới trái
		(0, 1, width + 1, width),# góc dưới phải
	]
	for bx, by in boxes:  # double box deadlock
		box = bx * width + by
		for pos in double_box_positions:
			pos_set = set()
			for dir in pos:
				pos_set.add(state[box + dir])
			if pos_set in ({'@', '+'}, {'@'}, {'@', '$'}, {'@', '$', '+'}):
				return True
	box = goal = 0
	for i in range(width + 1, 2 * width - 1):  # too many boxes deadlock
		if state[i] == '@':
			box += 1
		elif state[i] in 'X%':
			goal += 1
	if box > goal:
		return True
	box = goal = 0
	for i in range(width * (height - 2) + 1, width * (height - 2) + width - 1):
		if state[i] == '@':
			box += 1
		elif state[i] in 'X%':
			goal += 1
	if box > goal:
		return True
	box = goal = 0
	for i in range(width + 1, width * (height - 1) + 1, width):
		if state[i] == '@':
			box += 1
		elif state[i] in 'X%':
			goal += 1
	if box > goal:
		return True
	box = goal = 0
	for i in range(2 * width - 2, width * height - 2, width):
		if state[i] == '@':
			box += 1
		elif state[i] in 'X%':
			goal += 1
	if box > goal:
		return True
	return False


def can_move(state, shape, player_pos, move):
	new_state = list(state)
	x, y = player_pos
	_, width = shape
	move_cost = 0
	target = x + move[0], y + move[1]
	boxtarget = x + move[0] * 2, y + move[1] * 2
	curr1d = x * width + y
	target1d = target[0] * width + target[1]
	boxtarget1d = boxtarget[0] * width + boxtarget[1]
	if state[target1d] == '+':
		return None, move_cost
	elif state[target1d] in '-X':
		new_state[curr1d] = '-' if new_state[curr1d] == '*' else 'X'
		new_state[target1d] = '*' if new_state[target1d] == '-' else '%'
		move_cost = 3
	elif state[target1d] in '@$':
		if state[boxtarget1d] in '+@$':
			return None, move_cost
		elif state[boxtarget1d] in '-X':
			new_state[boxtarget1d] = '@' if new_state[boxtarget1d] == '-' else '$'
			new_state[target1d] = '*' if new_state[target1d] == '@' else '%'
			new_state[curr1d] = '-' if new_state[curr1d] == '*' else 'X'
			move_cost = 0 if new_state[boxtarget1d] == '$' else 2
	return ''.join(new_state), move_cost
# --- NEW: can_move_delta --- #
def can_move_delta(state, shape, player_pos, move, board):
	new_state = list(state)
	x, y = player_pos
	_, width = shape
	move_cost = 0
	target = x + move[0], y + move[1]
	boxtarget = x + move[0] * 2, y + move[1] * 2
	curr1d = x * width + y
	target1d = target[0] * width + target[1]
	boxtarget1d = boxtarget[0] * width + boxtarget[1]

	# default delta (không đẩy hộp)
	moved_from_idx = None
	moved_to_idx = None
	old_player_idx = board.to_idx[x, y]

	if state[target1d] == '+':
		return None, move_cost, None, None, None, None  # tường

	elif state[target1d] in '-X':
		new_state[curr1d] = '-' if new_state[curr1d] == '*' else 'X'
		new_state[target1d] = '*' if new_state[target1d] == '-' else '%'
		move_cost = 3
		new_player_idx = board.to_idx[target[0], target[1]]

	elif state[target1d] in '@$':
		if state[boxtarget1d] in '+@$':
			return None, move_cost, None, None, None, None  # hộp bị chặn
		elif state[boxtarget1d] in '-X':
			# cập nhật chuỗi
			new_state[boxtarget1d] = '@' if new_state[boxtarget1d] == '-' else '$'
			new_state[target1d] = '*' if new_state[target1d] == '@' else '%'
			new_state[curr1d] = '-' if new_state[curr1d] == '*' else 'X'
			move_cost = 0 if new_state[boxtarget1d] == '$' else 2
			new_player_idx = board.to_idx[target[0], target[1]]
			# delta hộp (from->to)
			moved_from_idx = board.to_idx[target[0], target[1]]  # ô có hộp trước khi người chơi bước vào
			moved_to_idx   = board.to_idx[boxtarget[0], boxtarget[1]]

	return ''.join(new_state), move_cost, (x + move[0], y + move[1]), old_player_idx, new_player_idx, (moved_from_idx, moved_to_idx)
