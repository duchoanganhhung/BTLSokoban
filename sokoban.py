...
## Đây là trình điều phối trung tâm: 
# tạo cửa sổ pygame và các widget giao diện(nút)
# Lắng nghe bàn phím, chuột, sự kiện
#Gọi tất cả các module khác
#Cập nhập hình ảnh, văn bản, trạng thái trò chơi

#Hàm chính chạy vòng đời của 1 màn chơi
...
# Kết thúc thiết lập
# Tạo đối tượng game(với level và window(là vùng không gian pygame vẽ toàn bộ hình ảnh text,...)) và thực hiện vòng lặp trò chơi
	game = Game(level=level, window=window)
	game_loop = True
	while game_loop:
		events = pygame.event.get() # Lấy tất cả event từ hàng đợi Pygame(kích chuột,...)
		for event in events:
		...
		if show_solution:
			widgets['paths'].draw()
		pygame.display.update() #Hiển thị phiên bản mới
